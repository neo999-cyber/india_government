...
w('# Derivations — the counts three records assert, recomputed from public data\n\n');
w(
...
w(
  '**Why this file exists.** Three records — L-0218, L-0219 and L-0220 — assert counts about the ' +
    'corpus itself and cite the repository as their source. For a reader that is an assertion on ' +
    'authority. This file replaces the assertion with a derivation wherever `/data` can carry one, ' +
...
w('---\n\n');
w(
  '**Nothing in this file re-cites a record.** A citation change may move a verdict and a 70-record ' +
    'decision is pending; pointing L-0219 and L-0220 at this table is a later step, taken deliberately.\n',
);

const body = P.join('');
// Self-description check: the rule strings must reach the page, or the file asserts a reproducibility
// it does not provide.
for (const needle of ['isBareRoot(u)', 'namesNoDocument(name)', 'THE DISCRIMINATOR']) {
