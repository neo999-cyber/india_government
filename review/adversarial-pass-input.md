...
Generated from the corpus at commit `1825b85` (2026-08-06) by `tools/gen-adversarial-pass-input.mjs`. Regenerating it against the same commit reproduces it byte for byte.
...
Every figure in this document that a gate emits is quoted from the block below, captured by running each gate at `1825b85`. Figures the document computes itself are printed with the field or needle that produced them. **The gates enforce internal consistency, not correctness** — they can prove a mark reaches a page and cannot prove the mark is true.

```
# Gate summary lines captured verbatim at 1825b85, 2026-08-06.
...
selftest OK — /data valid (165 warning(s))
...
Emitted by this generator from `/data` at `1825b85`. The field read is named in each line.
...
Checked at generation time: the classification's 67 ids are exactly the 67 records carrying `contested` at `1825b85`. No drift.
...
**Where a rule states a distribution, the measured value at `1825b85` is printed beside it. They do not always agree, and where they do not, both stand.**
...
**Measured at `1825b85`**, over 226 ledger records: `no-objective` 73 · `contested` 67 · `partly` 32 · `failed` 16 · `too-early` 13 · `baseline-context` 11 · `worked` 9 · `awaiting-adjudication` 4 · `reversed` 1.
...
**Measured at `1825b85`**, over 127 provenance records: `degrades-precision` 45 · `obscures` 31 · `disputed` 24 · `overstates-post-2014` 19 · `understates-post-2014` 6 · `understates-pre-2014` 1 · `overstates-pre-2014` 1.
...
**Measured at `1825b85`**: `not-published` 206 · `not-collected` 120 · `never-defined` 41 · `withheld` 12; disputed 19 (`evidentiary` 16 · `normative` 3).
...
**Found: 60 ledger and provenance records, and 6 series, at `1825b85`.** Full text is in the record itself — in Extract B for the 14 of these that are there, and in the field named below for the rest.
...
5. **The verification log**, 679 KB of cycle-by-cycle working notes. Where a decision's reasoning lives only there, it is not here.
