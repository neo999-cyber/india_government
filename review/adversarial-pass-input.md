...
Emitted by this generator from `/data` at `059912b`. The field read is named in each line.
...
Checked at generation time: the classification's 67 ids are exactly the 67 records carrying `contested` at `059912b`. No drift.
...
5. **The verification log**, 699 KB of cycle-by-cycle working notes. Where a decision's reasoning lives only there, it is not here.
