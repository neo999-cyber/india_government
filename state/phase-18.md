# Phase 18 — design lock. ARCHIVE.

**Closed material moved out of `drops/phase-18-design-lock/STATE.md` on 2026-08-11, as a pure
move.** Nothing here is edited, summarised or reordered: every line is byte-identical to what the
live file held, and the split asserted that the two partitions reconstruct the original exactly
before either was written.

**Why this exists.** The live file is read cold at the start of every session. Material that is
closed still costs that read, and a stale closed item is worse than an absent one — the queue below
was pinned at `ae0dcaa` and still described F2 to F5 as unstarted after all five had shipped.
**A section moves here only when it is closed, no open item depends on it, and any open item citing
it carries the evidence inline in the live file.**
