...
## 2026-08-11 — the adversarial data-model critique, tested against the corpus and acted on in part

An external tool reviewed the project and made five recommendations, opening with *"if your data is
flat, your UI will be flat."* **Each was checked against the corpus rather than accepted, and the
premise inverted: the data is the least flat part of this project — 30 fields on the ledger, four
layers, a 10-value assessment enum, three independence grades, 173 of 223 records carrying two or
more domains — and the VIEWS were flatter than the data.** Four of the five recommendations asked
the data model to fix a view problem.

**REJECTED ON THE FACTS — the narrative layer.** The critique asked for a \`summary\` field. It is
already REQUIRED in \`schemas/ledger.schema.json\`: **223 of 223 records carry one, median 508
characters**, beside \`whatHappened\`, \`claimAtLaunch\`, \`caseFor\`, \`caseAgainst\`,
\`assessmentNote\`, \`caveat\` and \`revisitTrigger\` — 17 prose fields, all at 0 invisible. Its
sibling \`significance\` stays absent deliberately: "why this record matters relative to others" is
a ranking, and rule 9 refuses rankings.

**DECLINED AS SPECIFIED — \`beneficiary\` and a lifecycle \`status\`.** farmer/student/MSME/women is
an inference unless the source states it, and **a facet returning 40 of a true 90 reads to a reader
as "there are 40"** — the fabricated-scope defect at UI scale, invisible to every gate there is. If
authored at all these are 223 new sourced claims, not a schema change. Recorded here so a later
cycle does not rediscover them as an obvious gap.

**BUILT — four things, of which the critique named two.**

**1. THE CORPUS IS SERVED AS DATA.** \`/data/pairs.json\` returned **404**; there were **zero**
JSON-LD tags across the build and zero downloads. 2.8 MB of schema-validated JSON, revalidated
every build, reachable only by \`git clone\`. Now \`/data/v1/\` publishes all four layers and the four
schemas beside them — **679 records, 2.60 MB** — generated by \`tools/gen-public-data.mjs\`, which
asserts every layer non-empty and every record id present BEFORE it deletes anything, and reads the
files back after. **The versioned path is the design, not decoration**: serving the schemas makes
them a public contract, so the manifest states in terms that an added enum value ships under v1 and
a removed, renamed or retyped one is v2. \`lib/data-contract.json\` is the single copy the manifest
and the \`/data/\` page both read — two statements of a promise is two promises. **No CSV**, and the
reason is rule 3a: flattening the ledger into columns is deciding which qualification survives.
JSON-LD on **620 pages** (\`CreativeWork\` for records, \`Dataset\` for series) carrying only
\`citation\`, \`isPartOf\` and \`dateModified\` — **no \`about\`, no \`sameAs\`, no \`aggregateRating\`**,
because each would assert a relationship the corpus does not hold, which is the causal-edge
rejection in another file format.

**2. THE CHANGELOG BECOMES VISIBLE, AND IT ALREADY EXISTED.** \`review/record-history.json\` has held
the full per-record git history since it was written and fed an internal selection routine and no
page. **220 of 353 records have been edited since publication and every one of them looked
untouched.** Now: *Has this record changed?* on every ledger and provenance page, and \`/corrections/\`
listing the **45 records whose verdict moved**, with the transition, the date and the commit.
**The edit count is worded "commits that touched this record" everywhere it appears** — it counts a
reserialisation and counts a four-field correction as one, so calling it "corrections" would be a
number nobody could re-derive. The verdict transitions are exact. **No error rate and no total
presented as one**, on rule 4b's ground: a share-corrected figure reads as a reliability score over
a sample this corpus selected.

**A deploy-safety defect found while wiring it and fixed before it could fire.** The generator
reconstructs history from \`git log\`, and **Vercel clones shallow** — a build there would have
regenerated the file from ten commits, reported success, and published "edited once" over a record
edited four times. A false claim about the record, from a green build, with nothing to contradict
it. The script now refuses to overwrite from a shallow checkout and says so; the committed file is
used unchanged on the deploy.

**3. FACETS, WHICH THE CRITIQUE MISSED BY AIMING AT THE SCHEMA.** There is no \`/search\` route and
there was no filter control anywhere — **faceted search was unavailable on the axes the corpus
already holds**, and \`app/ledger/page.tsx\` carried a scaffold note saying so. Closed on all three
listing surfaces by ONE component (the local-fix rule; the class is "listing surface", enumerated
from the routes as ledger, series, provenance, all three changed). **It filters rows that are
already server-rendered rather than rendering results** — three reasons, all pointing one way: rule
4b's marks must be in BUILT html or \`listing-marks\` cannot see them, rule 3a's caveat rows would
need a second renderer, and with JS off the reader gets the complete listing. Verified live:
223 → 21 education → 4 education+failed, caveat rows travelling with their rows, and the URL
carrying \`?domain=education&assessment=failed\` so a filtered view is linkable — a shared link lands
filtered with the controls restored. Options are derived from the records present, never from the
enum, so no second copy of an enum lands outside \`enum-parity\`'s reach.

**4. ENTITY RESOLUTION, AND THE CRITIQUE UNDERSTATED IT.** Measured: **546 distinct leading segments
across 1,205 citations**, with \`Ministry of Education\` / \`Ministry of Human Resource Development\`
sitting as two unlinked strings, \`MHA\` / \`Ministry of Home Affairs\` as two more (95 citations
between them), and \`Ministry of Commerce and Industry\` / \`Ministry of Commerce & Industry\`
differing by one character. **The reason this is a method problem and not a browse feature is
Ruling 1a, whose independence test is a test on the publisher** — and which is enforced today by a
human reading a free-text string. \`/publishers/\` resolves **598 of 1,205 (50%)** to 29 authored
entities, each carrying its ground, and **keeps the CARRIER apart from the BODY**: \`PIB / Ministry
of Power\` is body Ministry of Power, carried by PIB, because merging them would destroy the exact
distinction the resolution exists to serve. **84 citations are PIB-carried by leading segment across
all three layers** — scope stated because a looser scan for \`PIB\` anywhere in a ledger source name
returns 91, and the two answer different questions. **Nothing is merged on similarity**: the four
Finance Commissions share almost all of their names, are four constitutional bodies, and would be
collapsed by any resemblance rule. The 371 unresolved strings are published with the count, and two
classes are named — statutes and judgments, which have no body to find, and **parliamentary answers,
where the answering ministry is genuinely absent from the citation string.** That last is the one
real gap the resolution surfaces.

**A defect introduced by this batch, caught and swept.** \`.sr-only\` was written into two new
components while **no CSS rule defined it**, so the words "changed to" rendered visibly beside the
arrow they exist to replace. The class was named (visually-hidden text utility), its members
enumerated from the code (2, both new), and the utility added once. A second, adjacent class was
then swept the same way: a \`.label\` span followed by same-line text — **8 members, 5 broken and all
5 introduced by this batch, 3 pre-existing and all 3 already rendering the space**, which is
reported here because a sweep that lists only its finds cannot be told from one that did not run.

**Gates, each quoting its own emitted scope:** \`validate\` VALID, 0 errors · \`typecheck\` clean ·
\`no-unguarded-prose-field\` 21 prose + 53 non-prose · \`withdrawn-wording\` 29 corrections, 166
sibling comparisons · \`seam-span\` 127 spans · \`figure-consistency\` 18/18 · \`record-history\` 353
records over 111 commits, 45 with a changed verdict · \`public-data\` 679 records, 4 schemas, 2.60 MB
· \`reachability\` 1786/1786 across **668** pages · \`field-render-audit\` 4 layers, 0 invisible ·
\`link-check\` **27,975 hrefs across 668 pages, 19 route prefixes, 0 dead** (was 25,110 / 665 / 16) ·
\`listing-marks\` **2,630 rows, 3,713 marks** (was 2,600 / 3,667 — the corrections table adds 30
...
## 2026-08-11 (twenty-fourth entry) — THE CONTEXT SPLIT: CLAUDE.md into a rulebook and its reasons

No feature work, no prose, no records. One measurement, two splits, two rules.

### 1 — THE MEASUREMENT, AND THE ESTIMATE DID NOT HOLD

| file | assumed | measured |
|---|---|---|
| `CLAUDE.md`, charged every turn | 54 KB / ~14,000 tok | **121,636 B / ~30,409 tok** |
| live `STATE.md` | — | 20,231 B / ~5,060 tok |

**CLAUDE.md was 2.25× the size the brief assumed, and its own `Session cost` section said 54 KB.**
That figure was itself a correction made 2026-08-06 from a stale 22 KB. **Third generation of the
same defect, in the one section written to prevent it** — which is the argument for the rule that a
stated measurement must be re-measured or it becomes a claim about the past in the present tense.

Per-drop `STATE.md` files, none resident: phase-17 147 KB · environment-energy 322 KB ·
cycle-review 46 KB · phase-16 35 KB · foreign-trade 35 KB · federalism 32 KB · education 23 KB ·
phase-18 20 KB · kashmir-rights 14 KB · kashmir-security 5 KB.

### THE BREAKDOWN, AND WHY IT DECIDED THE METHOD

Classified at paragraph level over 201 paragraphs. **The first classifier was wrong and the way it
was wrong is worth recording**: it set a narrative flag at `### THE COUNTERFACTUAL ENGINE` and left
it on until the next `##`, which swallowed the mirror rule, the tier discipline and the prose-shadow
class — all live rules — into "narrative", reporting 32 per cent. That is the enumeration-scope
defect in a measuring script. Corrected to classify per paragraph:

| bucket | paras | bytes | share |
|---|---|---|---|
| **rule and evidence WELDED** | 88 | 68,569 | **57%** |
| imperative only | 63 | 26,917 | 22% |
| withdrawn wording | 7 | 10,389 | 9% |
| evidence only | 17 | 8,460 | 7% |
| neither (headings, preamble) | 23 | 5,999 | 5% |
| narrative | 3 | 669 | 1% |

**Then inside those 88 welded paragraphs, at sentence level:** 51 per cent pure rule, 16 per cent
pure evidence — movable whole — and **33 per cent welded inside a single sentence**, which cannot be
moved without rewriting the rule.

**THAT IS WHAT DECIDED THE METHOD. The file cannot be split mechanically.** Two mechanical
extractions were measured and rejected: moving whole evidence paragraphs gives 26 per cent, and
"keep every operative sentence" gives 17 per cent, because this file's prose uses *is not*, *read*
and *state* in evidence sentences as freely as in rules.

### 2 — THE SPLIT AS DONE: COPY VERBATIM, THEN AUTHOR AGAINST IT

**`docs/rules-earned.md` holds the pre-split CLAUDE.md in full, byte for byte**, split into its 186
natural rule blocks and keyed. Generated by `tools/gen-rules-earned.mjs`, which **asserts every
block round-trips verbatim before writing** and refuses otherwise. **Keys are derived from a block's
own opening text, not from its position**, so inserting a rule does not renumber every key and
invalidate every pointer.

CLAUDE.md was then authored fresh against it. **The safety property is that nothing was deleted:**
the evidence file is a copy, git holds the original, and every rule's imperative was carried across
rather than paraphrased.

**121,001 B → 51,457 B, a 57 per cent cut. 178 keys cited, all resolving; 8 blocks unpointed and all
8 are headings, which carry themselves.**

**A REAL DEFECT IN MY OWN WORK, CAUGHT BY THE CHECK AND NOT BY CARE: 33 of the keys I wrote were
INVENTED.** I authored pointers for the sections whose key index I had not dumped, producing
plausible six-hex keys that resolved to nothing. The verification — every cited key must exist in
the index — caught all 33, and each was remapped to its real block by reading the index. **A pointer
that looks right and resolves to nothing is worse than no pointer**, because it reads as a trace to
evidence that is not there; this is the fabricated-scope defect in a new position.

**WHICH RULES SHRANK TO A GATE POINTER**, reported as required. Only where the gate is total:
`validate` (rule 1), `enum-parity` (value lands in schema, type and label map), `no-bare-root`,
`phase-name`, `deploy-chain`, and `quotation-identity` **for the matching property only** — nothing
gates whether a correction was made at all, and that half stays in prose. **What did not shrink,
because no gate reaches it:** the prose-shadow class, assert-per-record, not-published versus
not-searched, selection-is-not-ranking, guard-binds-a-scope, the local-fix rule, the single-snapshot
rule, rulings 1–9, and the stop conditions. Those are rules precisely because nothing can check them.

### 3 — THE STATE.md ARCHIVE SPLIT, AS A PURE MOVE

Proposed in batch 19 and never done. `state/phase-18.md` now holds the closed material;
`drops/phase-18-design-lock/STATE.md` keeps the resume block, the one open item, the standing
hazards, the pins and an archive index.

**Asserted as a pure move before either file was written:** the partition of 209 lines into 184
moved and 25 kept **reconstructs the source exactly**; the moved block appears in the archive
verbatim; and **0 moved lines survive in the live file**. Numstat on the live file: **72 insertions,
184 deletions** — the 72 are the new resume, open item, pins and index, which are authored and not
moved, and are declared separately rather than folded into a "pure move" claim they would falsify.

**20,231 B → 5,465 B, a 73 per cent cut**, and it removed a live staleness: the queue was pinned at
`ae0dcaa` and still described F2 to F5 as unstarted after all five had shipped. **The open item
carries its evidence inline**, per the rule the proposal set — period counts, the governance-110 and
poverty-3 figures, the probe discipline and the evidence-note test are restated in the live file
because the section they came from is archived.

### 4 — TWO STANDING RULES WRITTEN IN

**DOM probes are authoritative; do not attempt screenshots.** The pane reports `document.hidden ===
true`; `setInterval` is clamped to ≥1000 ms, CSS transitions do not run, and **capture returns blank
after any scroll.** That has cost a diagnosis roughly six times and every attempt spends an image
plus the reasoning around it. One screenshot is worth it only above the fold immediately after
navigation.

**Delegate measurement to a subagent.** A sweep over the built site, a distribution count or a
codebase investigation reads many files and returns one number; that reading belongs in a separate
context. The rule requires the returned answer to name the scope it was computed over, so the number
is usable under the count-and-scope rule.

### 5 — BEFORE AND AFTER

| | before | after | saved |
|---|---|---|---|
| `CLAUDE.md`, every turn | 121,001 B / ~30,250 tok | 51,867 B / ~12,970 tok | **57%** |
| live `STATE.md`, cold start | 20,231 B / ~5,060 tok | 5,465 B / ~1,370 tok | **73%** |
| **orientation total** | **141,232 B / ~35,300 tok** | **57,332 B / ~14,300 tok** | **~21,000 tok** |

**CLAUDE.md is charged on every turn, so the saving multiplies by the turn count.** At forty turns
that is roughly 700,000 tokens of repeated cost removed from a batch.

Read on demand and never resident: `docs/rules-earned.md` 132 KB, `state/phase-18.md` 20 KB,
`docs/rules-earned.keys.json` 65 KB.

### Gate line

27 steps green. `phase-name` 391 → **395 tracked files**, 12 disagreeing assertions and 12 exempted
— the one new assertion is in `docs/rules-earned.md` and is exempted **for a stated reason**,
*governed by a supersession clause within the preceding window*, which is the block quoting the
withdrawn phase name inside the sentence that withdraws it. Checked rather than assumed, because
