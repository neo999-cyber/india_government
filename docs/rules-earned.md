# Rules earned — the evidence behind CLAUDE.md

**This file is not read at session start.** It is the other half of CLAUDE.md: every rule in
that file was earned by a defect, a measurement or an operator ruling, and this is where that
evidence lives now. CLAUDE.md keeps the imperative and one line of why, and points here by key.

**Nothing here is a paraphrase.** Every block below is the text as it stood in CLAUDE.md
immediately before the split of 2026-08-11, copied byte for byte and asserted by
`tools/gen-rules-earned.mjs`. **A rule with no recorded reason gets rationalised away by a
future pass — that has nearly happened twice — so the reasons must remain findable, and
findable means verbatim rather than remembered.**

Keys are derived from a block's own opening text, so inserting a rule does not renumber the
rest. To trace a rule: take its `[R-xxxxxx]` key from CLAUDE.md and search it here.
...
5a. **A denominator break is not a series break.** The 2022-23 rebasing lowered the level of nominal GDP by 3-4%, so every ratio-to-GDP (`fiscal-deficit`, `genl-govt-debt`, `exports-gdp`, `gfcf-gdp`) steps at 27 Feb 2026 with no change in underlying activity. Any chart or table of those spanning that date must mark it, and mark it **differently** from a series break: the seam is a solid red stop meaning do not splice, the denominator break is a **dashed brass band** meaning the line may continue but the values either side are divided by different things. Never render an arithmetic step as if it were activity. **CORRECTED 2026-08-11 with the Bone & Indigo palette: this read "a dashed umber band", and umber no longer exists.** The token it named (`--shift` #7a5c1e) folded into `--mark` brass, whose single sanctioned meaning — *a definition changed* — is what a restated denominator IS; keeping a second warm colour for the same idea would have cost a distinction nobody could see, since brass and umber sit 1.38 apart in contrast. **What the rule requires is unchanged and is not a colour**: the discriminator is SOLID against DASHED, plus words in both bands, so the two survive greyscale and survive a reader who cannot separate two warm hues. The marker is derived, not stored — a series qualifies when its unit is a ratio to GDP and it carries the restating provenance record — so no schema change is needed and none was made.
